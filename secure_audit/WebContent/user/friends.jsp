		...
		$(document).ready(function()
				{
					...
					$("#add_friend").hide();
						$("#search").click(function()
								{
									//alert("CLICKED");
										var search_item=$("#search_value").val();
										$("#user_img").show();
										$("#add_friend").show();
										if(search_item=="")
												{
											...
											{
												$.getJSON("friends_Action.jsp?searchkey="+search_item,function(json)
														{
													
															//alert("JSON");
															 var len=json.length;
															//alert(len); 
															var name=json[0].namekey;
															var image_path=json[1].namekey;
															//alert(name);
															//alert(image_path);
															 $("#user_name").html("").append(name);
															 //$("#user_img").prepend(image_path);
															 $("#user_img").attr("src",image_path);
															 
														});
												
												$("#add_friend").click(function()
														{
															var receiver=$("#search_value").val();
															var sender=$("#sender").val();
															//alert("Sender"+" "+sender);
															//alert("Rec"+" "+receiver);
															
															$.getJSON("sendReq_Action.jsp?senderkey="+sender+"&receiverkey="+receiver,function(json)
																	{
																		alert("JSON2");
																		var status=json[0].msg;
         ...
         <div class="form">
                 <form class="cmxform" id="contactForm" method="post" >
                    <div class="to1">
					 ...
					 <input type="text" class="text" id="search_value" placeholder="Search" name="search_value" onfocus="this.value = '';" onblur="if (this.value == '') {this.value = 'Search';}" style="float:left;">
					<br> <input type="button" id="search" name="search" value="Search">
					 </div>
					 <div class="frnds">
					 <img src="images/avatar.jpeg" id="user_img"> 
					 <span><p id="user_name"></p></span>
					 <span><input type="button" id="add_friend" value="SEND REQUEST"></span>
					 </div>
